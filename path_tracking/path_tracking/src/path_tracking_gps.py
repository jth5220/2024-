...
def latlon_to_utm(lat, lon):
    ...
    latlon_to_utm_ = pyproj.Proj(proj, preserve_units=True)
    return latlon_to_utm_(lon, lat)
...
class PathTracking():
    def __init__(self):
        ...
        self.time_plot_3 = TimePlot(data_name=['utm_x', 'utm_y', 'cur_speed','target_predicted_speed'],
                                    title='Speed record')
        # ROS
        rospy.init_node('path_tracker', anonymous=True)
        
        self.position_sub = rospy.Subscriber("/ublox_gps/fix", NavSatFix, self.callback_position)
    ...
    def callback_position(self, gps_msg):
        self.x_utm, self.y_utm = latlon_to_utm(gps_msg.latitude, gps_msg.longitude)
        return
    ...
    def callback_path_tracking(self, path_msg, speed_msg):
        ...
        final_speed += 0.3 # 억지로 더 넣어주면 브레이크가 잡아줌?
        
        # time plot
        self.time_plot_2.update([speed, target_speed, final_break/10, target_speed_filtered])
        
        if self.x_utm is not None:
            self.time_plot_3.update([self.x_utm, self.y_utm, speed, target_speed_filtered])
...
if __name__ == '__main__':
    ...
    path_tracking.time_plot_3.save_csv('SpeedRecord1009.csv')
