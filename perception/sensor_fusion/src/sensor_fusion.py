...
class SensorFusion():
    def __init__(self):
        ...
        self.fusion_pub = rospy.Publisher("/sensor_fusion/rubber_cones", MarkerArray, queue_size=10)
        # self.fusion2_pub = rospy.Publisher("/sensor_fusion/unmatched_rubber_cones", MarkerArray, queue_size=10)
    ...
    def callback_fusion(self, cluster_msg, bbox_msg):
        ...
        clusters_2d_left, valid_indicies_left = projection_3d_to_2d(clusters, self.intrinsic_left, self.extrinsic_left)
        clusters_2d_right, valid_indicies_right = projection_3d_to_2d(clusters, self.intrinsic_right, self.extrinsic_right)
        ...
        matched_left = hungarian_match(clusters_2d_left, left_bboxes, left_bboxes_label, distance_threshold=120)
        matched_right = hungarian_match(clusters_2d_right, right_bboxes, right_bboxes_label, distance_threshold=120)
        
        labels_left = get_label(matched_left, valid_indicies_left)
        labels_right = get_label(matched_right, valid_indicies_right)
        labels = [i if i != -1 else j if j != -1 else -1 for i, j in zip(labels_left, labels_right)]
