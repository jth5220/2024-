...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2_in) {
  ...
  *pcl_pc_in += *outskirt;
  ...
  pt.setFilterLimits(z_axis_min_, z_axis_max_); // 0.2
  pt.filter(*pcl_pc_in);
  ...
  for(int i = 0; i < region_max_; i++) {
    tolerance += tolerance_offset; //0.067
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "adaptive_clustering_no_ground_removed");
  ...
  private_nh.param<float>("x_axis_min", x_axis_min_, -3.0);
  private_nh.param<float>("x_axis_max", x_axis_max_, 6.0); // 0.1
  private_nh.param<float>("y_axis_min", y_axis_min_, -4.0);
  private_nh.param<float>("y_axis_max", y_axis_max_, 4.0); // 0.1
  private_nh.param<float>("z_axis_min", z_axis_min_, -0.5);
  private_nh.param<float>("z_axis_max", z_axis_max_, 0.5); // 0.1
  private_nh.param<int>("cluster_size_min", cluster_size_min_, 3);
  private_nh.param<int>("cluster_size_max", cluster_size_max_, 10000);
  private_nh.param<float>("tolerance_offset", tolerance_offset, 0.1);
