...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& ros_pc2_in) {
  ...
  pt.setFilterLimits(-0.65, 1.5); // 0.2
  pt.filter(*pcl_pc_in);
  
  pt.setInputCloud(pcl_pc_in);
  pt.setFilterFieldName("y");
  pt.setFilterLimits(-4, 4);
  pt.filter(*pcl_pc_in);
  ...
  for(int i = 0; i < clusters.size(); i++) 
  {
    ...
    if(marker_array_pub_.getNumSubscribers() > 0) 
    {
      ...
      if (size_z > 0.2 && size_x <1 && size_y <1 ) 
      {
      ...
      if ( size_z > 0.2 && size_x <1 && size_y <1 && size_z <1.0 ) // 라바콘을 잡기 위한 파라미터
      {
...
int main(int argc, char **argv) {
  ...
  private_nh.param<float>("z_axis_max", z_axis_max_, 0.1); // 0.1
