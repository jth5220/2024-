...
class TargetSpeedUpdate():
    def __init__(self):        
        ...
        # Graph Plotting
        self.data_plot = DataPlot(data_name=['current speed', 'target speed','Final speed', 'brake'],\
        ...
        # PID controller
        self.pid = PID(kp=5.5, ki=0.0, kd=0.0)
    ...
    def callback_speed_PID(self, speed_msg):     
        # Target speed (0 -> 3 -> 2)
        if time.time()-self.first_time < 2:
            self.target_speed = 0.0
        elif time.time()-self.first_time < 7:
        ...
        # elif (cur_speed - target_speed) <= 0.5: 
        #     final_speed = target_speed
        #     final_break = 1
            
        elif cur_speed - target_speed >= 0.5: # elif final_speed <= 0:
            final_speed = 0
            # final_break = abs(PID_term[0] * 15) # kp * error^2 * 10
            final_break = abs(PID_term[0]**2/4.0 * 8) # kp * error^2 * 10
        
        elif 0 <= cur_speed - target_speed <= 0.5:
            final_speed = target_speed
            final_break = 1
        ...
        # Data for plot
        self.data_plot.update([cur_speed, target_speed, final_speed, final_break / 10])
